...
export async function GET(request: Request) {
  ...
  try {
    const response = await fetch(`http://14.139.187.229:8081/univault/get_mcq_test_review.php?test_result_id=${testResultId}`, {
      headers: {
    ...
    console.error('Failed to fetch MCQ test review via proxy:', error);
    return NextResponse.json(
